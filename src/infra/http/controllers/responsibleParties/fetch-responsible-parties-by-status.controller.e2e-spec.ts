...
describe('Fetch Responsible Parties by Status  (E2E)', () => {
  ...
  test('[GET] /responsible-parties/:status', async () => {
    ...
    const response = await request(app.getHttpServer())
      .get(`/responsible-parties/active`)
      .set('Authorization', `Bearer ${accessToken}`)

    // console.log(response);

    expect(response.statusCode).toBe(200)
    expect(response.body.responsibleParties).length(1)
  })
})
